...
headless 모드로 실행한다. GUI를 열지 않으며 각 시나리오의 수치 지표를 기대 범위와 비교한 뒤
HTML과 Markdown 보고서를 생성한다.
...
``report.md`` 는 전체 PASS/FAIL/SKIP 수와 PDF별 상태/실행 시간을 표로 제공한다. 동일한 출력
폴더로 다시 실행하면 두 파일은 최신 결과로 덮어쓴다.
